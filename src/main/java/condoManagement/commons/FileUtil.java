...
import static condoManagement.commons.AppUtil.checkArgument;
...
public class FileUtil {
    ...
    public static String getFileType(String filePath) throws IOException {
        ...
        if (lastDot == -1) {
            throw new IOException("THE FILE MUST HAVE A FILE EXTENSION.");
        } else {
     ...
     */
    public static void copyImage(String image, File toSave) throws IOException {
        try {
            copyFile(image, toSave);
        } catch (IOException ioe) {
            throw new IOException("IMAGE FILE COULD NOT BE COPIED.");
        }
    }
}
